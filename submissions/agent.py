...
def report_node(state: AnalysisState) -> dict:
    ...
    prompt = PromptTemplate(
        ...
        template="""
请基于以下QAR数据分析结果生成专业的Markdown报告，结构如下：
        1. 数据概述（QAR数据基本信息+清洗过程，突出飞行阶段过滤和行业阈值应用）
        2. 飞行参数分析
           - 关键参数时间序列趋势（高度、速度、发动机转速等）
           - 不同飞行阶段（起飞/巡航/降落）的参数分布差异
        3. 统计检验结果
           - 发动机参数与飞行参数的关联性（重点解读推力-速度相关性）
           - 飞行阶段间参数差异的显著性（ANOVA结果）
        4. 核心洞见
           - 合规性评估：参数是否在民航标准范围内
           - 发动机健康状态：异常值出现频率与阶段分布
           - 飞行操作优化点：基于参数关联性的效率提升建议
        5. 建议措施（针对异常值和低效率参数提出具体检查/调整建议）
        ...
        要求：
        - 所有结论需结合民航QAR分析规范，避免通用化描述
        - 异常值解读需区分传感器故障与实际操作异常
        - 建议部分需具体到"检查XX传感器"、"优化XX阶段的推力设置"等可执行动作
        """
    )
